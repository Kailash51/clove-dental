  ...
  if (consent && submitBtn) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (form.matches(".form")) {
        ...
        if (
          !capInput ||
          capInput.value.trim() !== captchaText.textContent.trim()
        ) {
          ok = false;
...
document.querySelectorAll(".js-buy").forEach((btn) => {
  btn.addEventListener("click", () => {
    alert(
      "Plan added to cart (demo). Proceed to checkout in your implementation."
    );
  });
});

// Simple interaction for the form checkbox
document
  .querySelector(".group-2__rectangle-3045")
  .addEventListener("click", function () {
    this.style.backgroundColor =
      this.style.backgroundColor === "orange" ? "white" : "orange";
  });

// Form submission handling
document
  .querySelector(".group-2__frame-26094932")
  .addEventListener("click", function () {
    alert(
      "Thank you for booking a free consultation! We will contact you shortly."
    );
  });
  
function validateCaptcha(event) {
  var captchaInput = document.querySelector(".captcha-input").value.trim();
  var captchaValue = document
    .querySelector(".captcha-value")
    .textContent.trim();
